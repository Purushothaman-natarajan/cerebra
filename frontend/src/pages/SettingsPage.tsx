/** Settings page — responsive layout with general, execution, notifications, danger zone. */
...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <h1 className="text-xl sm:text-2xl font-bold text-foreground">⚙️ Settings</h1>
              ...
              {(["light", "dark", "system"] as const).map((t) => (
                <button key={t} onClick={() => setTheme(t)}
                  className={`px-4 py-2 text-sm rounded-lg border transition-all duration-200 ${
                    theme === t ? "bg-accent text-white border-accent shadow-sm" : "border-border hover:bg-accent-soft"
                  }`}
                >
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Input label="Token Budget" type="number" defaultValue={10000} className="sm:col-span-2" />
        </div>
      </Card>

      <Card>
        <h2 className="font-semibold text-foreground mb-4">Notifications</h2>
        {["Notify on run failure", "Notify on run complete"].map((n) => (
          <label key={n} className="flex items-center gap-3 text-sm cursor-pointer p-2 rounded-lg hover:bg-accent-soft transition-colors">
            <input type="checkbox" defaultChecked={n.includes("failure")} className="rounded border-border accent-accent w-4 h-4" />
