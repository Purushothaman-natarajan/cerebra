/** Templates page — browse pre-built workflows with history tracking. */
          ...
          <div className="space-y-4">
            ...
            <div className="border border-border rounded-xl p-3 bg-surface">
              ...
              <div className="flex items-center gap-1.5 text-xs text-muted flex-wrap">
                {selected.nodes.filter((n: { type: string }) => n.type === "agent").map((n: { id: string }, i: number) => (
                  <span key={n.id} className="flex items-center gap-1">
                    <span className="px-2 py-0.5 rounded bg-accent-soft text-accent text-[10px]">{n.id}</span>
                    {i < selected.nodes.filter((n: { type: string }) => n.type === "agent").length - 1 && <ArrowRight className="w-3 h-3" />}
                  </span>
                ))}
              </div>
            </div>
