  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Select
        ...
        onChange={(e) => update("tool_type", e.target.value as ToolFormData["tool_type"])}
        options={[
      ...
      {form.tool_type === "python" && (
        <Textarea label="Python Code" value={form.config.code ?? ""} onChange={(e) => updateConfig("code", e.target.value)} rows={8} placeholder='async def run(params: dict) -> str:\n  return f"Hello {params.get("name", "world")}"' />
      )}
